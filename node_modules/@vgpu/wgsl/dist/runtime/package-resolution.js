...
function packageNotFound(pkg, fixit) {
...
export function resolveImport(spec, from, opts, diagnostics) {
    ...
    if (opts.modules)
        throw packageNotFound(packageNameOf(spec), PKG_NOTFOUND_VIRTUAL_FIXIT);
    if (spec.startsWith("./") || spec.startsWith("../"))
        return defaultFile(resolve(dirname(from), spec));
    return packageImport(spec, from, diagnostics);
}
export async function readModule(path, opts) {
 ...
 */
function walkForPackage(start, stopAt, pkg, sub, diagnostics) {
    for (let dir = start;;) {
...
function workspaceBoundary(start) {
    let dir = start;
...
function resolveFromImporter(spec, from) {
    try {
        return defaultFile(createRequire(from).resolve(spec));
    }
    catch {
        return undefined;
    }
 ...
 */
function resolveAlongsideResolver(spec) {
    try {
