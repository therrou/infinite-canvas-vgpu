...
export function emitModule(module, exportsByPath, pathOf) {
    ...
    for (const local of module.parsed.locals)
        if (!isVisible(local.kind, module, local.name))
 ...
 */
function shadowedTokens(tokens) {
    const shadowed = new Set();
    const hide = (name, start, end) => { for (let i = start; i <= end; i++)
    ...
    for (let i = 0, depth = 0; i < tokens.length; i++) {
        ...
        else if (depth === 0 && text === "fn")
            i = hideFunctionLocals(tokens, i, hide);
    }
    return shadowed;
}
/** Parameters shadow the body only; a local shadows from its own `;` to the end of its block. */
function hideFunctionLocals(tokens, fnIndex, hide) {
    ...
    if (open === undefined || close === undefined || bodyOpen === undefined || bodyClose === undefined)
        return fnIndex;
    ...
    for (let i = open + 1; i < close; i++)
        if (tokens[i].kind === "ident" && tokens[i + 1]?.text === ":")
            hide(tokens[i].text, bodyOpen, bodyClose);
    const scopeEnds = [bodyClose];
    for (let i = bodyOpen + 1; i < bodyClose; i++) {
        while (scopeEnds.length > 1 && i > scopeEnds[scopeEnds.length - 1])
            scopeEnds.pop();
        const text = tokens[i].text;
        // A `for` frame spans header plus body so a loop variable does not leak past the loop.
        if (text === "for" || text === "{") {
            scopeEnds.push(blockEnd(tokens, i, scopeEnds[scopeEnds.length - 1]));
            continue;
        }
        if (text !== "let" && text !== "var" && text !== "const")
            continue;
        const name = localNameIndex(tokens, i);
        if (name === undefined || name >= bodyClose)
            continue;
...
function blockEnd(tokens, index, fallback) {
    if (tokens[index].text === "{")
        return matchPair(tokens, index, "{", "}") ?? fallback;
...
function localNameIndex(tokens, kindIndex) {
    let i = kindIndex + 1;
    if (tokens[i]?.text === "<") {
...
function seek(tokens, start, text) { for (let i = start + 1; i < tokens.length; i++)
    if (tokens[i].text === text)
        return i; return undefined; }
function matchPair(tokens, openIndex, open, close) { let depth = 0; for (let i = openIndex; i < tokens.length; i++) {
...
function isVisible(kind, module, name) { return kind === "override" || isEntryPoint(module, name) || isBindingVar(module, name); }
function isBindingVar(module, name) {
    for (let i = 0; i < module.tokens.length; i++) {
        if (module.tokens[i]?.text !== "var")
            continue;
        const nameIndex = varNameIndex(module.tokens, i);
        if (module.tokens[nameIndex]?.text !== name)
            continue;
        for (let j = i - 1; j >= 0 && module.tokens[j]?.text !== ";" && module.tokens[j]?.text !== "}"; j--) {
            if (module.tokens[j]?.text === "@" && ["group", "binding"].includes(module.tokens[j + 1]?.text ?? ""))
                return true;
        }
    }
    return false;
}
function varNameIndex(tokens, varIndex) {
    if (tokens[varIndex + 1]?.text !== "<")
        return varIndex + 1;
    let depth = 0;
