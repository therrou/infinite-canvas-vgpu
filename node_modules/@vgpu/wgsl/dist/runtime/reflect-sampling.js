import { analyzeWgslTokens } from "./scope-walker.js";
import { numericAttr } from "./reflect-utils.js";
...
export function entrySamplingPairs(modules, raw, bindings) {
    ...
    for (let moduleIndex = 0; moduleIndex < modules.length; moduleIndex++) {
        ...
        for (const variable of decls.vars) {
            ...
            if (group !== undefined && binding !== undefined && declaration)
        ...
        for (const entry of decls.entries) {
            ...
            if (!fallback && root)
                fallback = !walk(root.id, new Map(), new Set(), analysis, bindingDeclarations, functionDeclarations, pairs);
...
function walk(functionId, env, visited, analysis, globals, functions, output) {
    ...
    for (let i = fn.bodyStartToken + 1; i < fn.bodyEndToken; i++) {
        ...
        if (!mode && callee === undefined)
            continue;
        const open = nextSignificant(analysis, i);
        if (open === undefined || analysis.tokens[open]?.text !== "(")
            continue;
...
function resolveOrigin(start, end, analysis, globals, env) {
    for (const ref of analysis.references) {
        if (ref.tokenIndex < start || ref.tokenIndex > end)
            continue;
...
function staticallyUsedBindings(root, analysis, globals, functions) {
    const pending = [root], visited = new Set(), used = new Map();
...
function ref(binding) { return { group: binding.group, binding: binding.binding }; }
function dedupe(pairs) {
    const seen = new Set();
    return pairs.filter((pair) => { const key = `${pair.texture.group}:${pair.texture.binding}:${pair.sampler.group}:${pair.sampler.binding}:${pair.mode}`; if (seen.has(key))
        return false; seen.add(key); return true; });
}
//# sourceMappingURL=reflect-sampling.js.map