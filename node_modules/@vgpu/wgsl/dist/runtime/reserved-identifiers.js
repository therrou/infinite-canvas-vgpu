...
import { WGSL_RESERVED_WORDS, WGSL_SPEC_KEYWORDS } from "./wgsl-identifiers.js";
...
export function reservedIdentifierDiagnostics(module) {
    ...
    while (i < tokens.length) {
        ...
        if (kind === "struct") {
            i = visitStruct(tokens, i, report);
            continue;
        }
        if (kind === "fn") {
            i = visitFunction(tokens, i, report);
            continue;
        }
        if (kind === "alias") {
            report(i + 1, "type alias");
            i = statementEnd(tokens, i) + 1;
            continue;
        }
 ...
 */
export function reservedIdentifierDiagnosticsForSource(path, source) {
    try {
...
function visitStruct(tokens, index, report) {
...
function visitFunction(tokens, index, report) {
    ...
    for (let i = bodyOpen + 1; i < bodyClose; i++) {
        ...
        if (token.kind !== "keyword")
            continue;
        if (token.text !== "let" && token.text !== "var" && token.text !== "const")
            continue;
...
function visitDeclarationList(tokens, start, end, site, report) {
    let i = start;
    while (i < end) {
        i = skipAttributes(tokens, i);
        if (i >= end)
...
function declarationListItemEnd(tokens, start, end) {
    let angle = 0;
...
function findText(tokens, start, text) {
    for (let i = start; i < tokens.length; i++)
        if (tokens[i].text === text)
            return i;
    return -1;
}
function matchingIndex(tokens, open) {
    ...
    for (let i = open; i < tokens.length; i++) {
        if (tokens[i].text === start)
            depth++;
        else if (tokens[i].text === end && --depth === 0)
            return i;
    }
    return -1;
}
function reservedIdentifierDiagnostic(path, token, site) {
    if (!token || (token.kind !== "ident" && token.kind !== "keyword"))
        return undefined;
    const reserved = WGSL_RESERVED_WORDS.has(token.text);
    if (!reserved && !WGSL_SPEC_KEYWORDS.has(token.text))
        return undefined;
    ...
    const diagnostic = wgslError(RESERVED_IDENTIFIER_CODE, message, token.line, token.column);
