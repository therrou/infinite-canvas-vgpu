...
export const softwareRendererExpectedHashes = {
...
export function softwareRendererCacheDirectory(options = {}) {
...
export function softwareRendererIcdPath(options = {}) {
    return join(softwareRendererCacheDirectory(options), "lvp_icd.json");
}
export function softwareRendererArchivePath(options = {}) {
    return join(softwareRendererCacheDirectory(options), `mesa-lavapipe-25.0.7-linux-${options.arch ?? process.arch}.tar.gz`);
}
export function softwareRendererExpectedHash(arch) {
    const hash = softwareRendererExpectedHashes[arch];
    if (!hash)
        throw softwareRendererError("VGPU-NODE-SOFTWARE-RENDERER-UNSUPPORTED", `No portable software renderer checksum is pinned for ${arch}.`);
    return hash;
}
export function getCachedSoftwareRenderer(options = {}) {
    ...
    if (![archive, icd, library].every(existsSync))
        return null;
    for (const path of [archive, icd, library])
...
export function verifySoftwareRendererFiles(directory, arch = process.arch) {
...
export function verifySoftwareRendererArchive(path, expected) {
    assertSoftwareRendererRegularFile(path);
    verifyFileHash(path, expected);
}
function verifyFileHash(path, expected) {
    const actual = createHash("sha256").update(readFileSync(path)).digest("hex");
    if (actual !== expected)
        throw softwareRendererError("VGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer ${path}: SHA-256 ${actual} does not match pinned ${expected}.`);
}
export function createPrivateSoftwareRendererCopy(icd, arch = process.arch) {
...
function copyVerified(source, destination, expected, mode) {
    ...
    if (actual !== expected)
        throw softwareRendererError("VGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer ${source}: SHA-256 ${actual} does not match pinned ${expected}.`);
}
export function assertSoftwareRendererRegularFile(path) {
    const stats = lstatSync(path);
    if (stats.isSymbolicLink() || !stats.isFile())
        throw softwareRendererError("VGPU-NODE-SOFTWARE-RENDERER-CHECKSUM", `Refusing software renderer cache path ${path}: it is not a regular non-symlink file.`);
}
export function softwareRendererError(code, message, cause) {
...
//# sourceMappingURL=software-renderer-cache.js.map