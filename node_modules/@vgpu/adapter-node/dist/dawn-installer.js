...
import { accessSync, chmodSync, closeSync, constants, createWriteStream, existsSync, lstatSync, mkdirSync, mkdtempSync, openSync, readFileSync, readSync, renameSync, rmSync, writeSync, } from "node:fs";
...
export function dawnCachePath(options = {}) {
...
export function verifyDawnBinary(path, expectedSha256 = dawnAssetSha256) {
    assertRegularNonSymlink(path);
    const actual = createHash("sha256").update(readFileSync(path)).digest("hex");
    assertHash(path, actual, expectedSha256);
}
export function getCachedDawnBinary(options = {}) {
 ...
 */
export function createPrivateDawnCopy(source, expectedSha256) {
    ...
    try {
        ...
        for (;;) {
            ...
            let offset = 0;
            while (offset < count)
                offset += writeSync(destinationFd, buffer, offset, count - offset);
        }
        if (expectedSha256)
            assertHash(source, hash.digest("hex"), expectedSha256);
...
export async function installDawn(options = {}) {
    ...
    if (!fetchImpl)
        throw prebuildMissing("blocked: this Node.js runtime does not provide fetch", undefined, platform, arch);
    ...
    const request = (url, init) => fetchWithDeadline(fetchImpl, url, init, Math.min(requestTimeoutMs, remainingMs(startedAt, overallTimeoutMs)));
    ...
    try {
        ...
        if (!response.ok || !response.body)
            throw httpError("GitHub release asset", response.status, response.statusText);
        ...
        try {
            ...
            await withTimeout(pipeline(Readable.fromWeb(response.body), stream, { signal: bodyController.signal }), remainingMs(startedAt, overallTimeoutMs), "Dawn response body", () => bodyController.abort());
        ...
        if (cause instanceof VGPUError)
            throw cause;
        throw prebuildMissing(downloadFailureReason(cause), cause, platform, arch);
    }
}
export function getUnsupportedPrebuildReason(options = {}) {
    const platform = options.platform ?? process.platform;
    const arch = options.arch ?? process.arch;
    if (platform !== "linux")
        return `unsupported platform ${platform}/${arch}; vgpu prebuilds currently target Linux arm64 only`;
    if (arch !== "arm64")
        return `unsupported platform ${platform}/${arch}; no vgpu prebuild is published for this CPU yet`;
    const libc = options.libc ?? detectLinuxLibc();
    if (libc === "musl")
        return "musl libc is unsupported; this prebuild requires glibc 2.31 or newer";
    if (libc !== "glibc")
        return "libc could not be identified as glibc; musl and unknown libc runtimes are unsupported";
    return null;
}
function publishCacheFile(temporary, destination, expectedSha256) {
    try {
...
async function resolveAuthenticatedAsset(request, token, remaining) {
    ...
    if (!response.ok)
        throw httpError("GitHub releases API", response.status, response.statusText);
    const releases = (await withTimeout(response.json(), remaining(), "GitHub releases response body"));
    const release = releases.find((candidate) => candidate.tag_name === dawnReleaseTag);
    if (!release)
        throw new Error(`release ${dawnReleaseTag} was not found (HTTP 404 equivalent)`);
    const asset = release.assets?.find((candidate) => candidate.name === dawnAssetName);
    if (!asset?.url)
        throw new Error(`release ${dawnReleaseTag} does not contain asset ${dawnAssetName} (HTTP 404 equivalent)`);
...
function withTimeout(promise, timeoutMs, operation, onTimeout) {
...
function remainingMs(startedAt, overallTimeoutMs) {
    const remaining = overallTimeoutMs - (Date.now() - startedAt);
    if (remaining <= 0)
        throw new Error(`Dawn download exceeded its ${overallTimeoutMs}ms overall deadline`);
    return remaining;
}
function assertCacheLocationWritable(cacheDirectory) {
    ...
    try {
        ...
        if (!stats.isDirectory() || stats.isSymbolicLink())
            throw new Error("nearest existing ancestor is not a real directory");
...
function detectLinuxLibc() {
    ...
    try {
        ...
        if (report?.header?.glibcVersionRuntime)
            return "glibc";
        if (report?.sharedObjects?.some((path) => /(?:^|\/)ld-musl-|libc\.musl-/u.test(path)))
            return "musl";
    }
    catch {
...
function assertRegularNonSymlink(path) {
    ...
    if (stats.isSymbolicLink())
        throw checksumError(path, "symbolic links are not accepted");
    if (!stats.isFile())
        throw checksumError(path, "the path is not a regular file");
}
function assertHash(path, actual, expected) {
    if (actual !== expected)
        throw checksumError(path, `SHA-256 ${actual} does not match pinned ${expected}`);
}
function checksumError(path, reason, cause) {
...
function httpError(scope, status, statusText) {
    if (status === 404)
        return new Error(`${scope} was not found (HTTP 404 ${statusText}); download failed or was blocked`);
    if (status >= 500)
        return new Error(`${scope} service unavailable (HTTP ${status} ${statusText})`);
    return new Error(`${scope} returned HTTP ${status} ${statusText}`);
}
function downloadFailureReason(cause) {
    const text = String(cause);
    if (/timed out|deadline/iu.test(text))
        return `download timed out or was blocked: ${text}`;
    if (/ENOTFOUND|EAI_AGAIN|ENETUNREACH|ECONNREFUSED|fetch failed|network|offline/iu.test(text))
        return `offline: ${text}`;
    if (/HTTP 404|not found/iu.test(text))
        return `download failed or was blocked: release or asset not found: ${text}`;
    if (/HTTP 5\d\d|service unavailable/iu.test(text))
        return `GitHub service unavailable: ${text}`;
    if (/HTTP (?:401|403|407|429)|blocked/iu.test(text))
        return `blocked by authentication, proxy, or rate limit: ${text}`;
    return `download failed or was blocked: ${text}`;
}
function prebuildMissing(reason, cause, platform, arch) {
    return new VGPUError({
...
//# sourceMappingURL=dawn-installer.js.map