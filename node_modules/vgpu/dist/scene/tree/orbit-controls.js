...
import { invertAffineMat4, transformPoint } from "./math.js";
...
export class OrbitControls {
    ...
    constructor(node, options = {}) {
        ...
        if (options.target !== undefined)
            writeVec3(this.#target, options.target, "target", this.label ?? "orbitControls");
    ...
    set(values) {
        ...
        if (values.target !== undefined)
            writeVec3(this.#target, values.target, "target", where);
        if (values.yaw !== undefined)
            this.#yaw = this.#goalYaw = values.yaw;
        if (values.pitch !== undefined)
            this.#pitch = this.#goalPitch = clamp(values.pitch, this.#minPitch, this.#maxPitch);
        if (values.distance !== undefined)
            this.#distance = this.#goalDistance = clamp(values.distance, this.#minDistance, this.#maxDistance);
        this.#applied = false;
        return this;
    }
    /**
    ...
    update(deltaTime = DEFAULT_DELTA_TIME) {
        ...
        const gap = Math.max(Math.abs(this.#goalYaw - this.#yaw), Math.abs(this.#goalPitch - this.#pitch), Math.abs(this.#goalDistance - this.#distance));
...
export function orbitControls(node, options = {}) {
