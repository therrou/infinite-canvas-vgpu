import { timerCapacityError, timerInvalidError } from "./errors.js";
import { FRAME_PASS_ATTACHMENT } from "./frame-protocols.js";
import { liveKernel, ownQueryFeature } from "./live-kernel.js";
import { createQueryRing } from "./query-ring.js";
...
export class InternalTimer {
     ...
     */
    #frameRetains = new Map();
    #disposed = false;
    constructor(device, host = {}) {
        ...
        if (!device.features.has("timestamp-query")) {
            throw timerInvalidError(`timer(gpu) needs the "timestamp-query" device feature — request it at init: init({ requiredFeatures: ["timestamp-query"] }).`, `Pass init({ requiredFeatures: ["timestamp-query"] }) on an adapter that supports it; gate optional timing on gpu.device.features.has("timestamp-query").`);
        }
        this.#device = device;
        this.#host = host;
     ...
     */
    attachSpan(span, frame, frameDevice) {
    ...
    finalizeFrame(frame, encoder) {
        this.#encodedSpans = undefined;
        if (this.#disposed || frame !== this.#frame || this.#usedQueries === 0)
            return;
        if (this.#ring.encodeResolve(encoder, this.#usedQueries))
            this.#encodedSpans = [...this.#frameSpans];
    }
    ...
    #retainRing(frame) {
        if (this.#frameRetains.has(frame))
            return;
    ...
    #assertUsable(where) {
        if (this.#disposed)
            throw timerInvalidError("the timer is disposed.", "Create a new timer with timer(gpu).", where);
    }
}
